...
# ------------------- Gemini API ---------------------------
GEMINI_API_KEY = "AIzaSyAK-YtMuo-XZjlJNrcr4xMGb9o46GoSGCM"  # ⚠️ replace with your actual key
...
def get_trends():
    ...
    for r in rows:
        if r.is_expense and r.ts.strftime("%Y-%m") == last_month:
            cat_breakdown[r.category] = cat_breakdown.get(r.category, 0) + r.amount

    return {
        "labels": labels,
        "income": income_by_month,
        "expense": expense_by_month,
        "category_breakdown": cat_breakdown  # now a normal dict
    }
...
async def analyze(payload: dict = Body(...)):
    ...
    if not question:
        raise HTTPException(status_code=400, detail="text or question is required")
