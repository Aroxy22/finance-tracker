...
tokenizer = AutoTokenizer.from_pretrained("Rustamshry/Qwen3-1.7B-finance-reasoning")
model = AutoModelForCausalLM.from_pretrained("Rustamshry/Qwen3-1.7B-finance-reasoning")

@app.post("/api/ask")
def ask_finance(payload: AskPayload):
    # Include last 100 transactions as context
    sess = SessionLocal()
    txs = sess.query(Transaction).order_by(Transaction.ts.desc()).limit(100).all()
