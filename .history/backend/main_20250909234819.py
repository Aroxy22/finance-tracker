...
def analyze(payload: AskRequest):
    question = payload.question or payload.text
    if not question:
        raise HTTPException(status_code=400, detail="question is required")

    # Simple stub (replace with Gemini if you want real AI)
    tips = [
        "Try to reduce food delivery spending by 15%.",
        "Automate your savings by setting a monthly transfer.",
        "Cancel unused subscriptions to save money.",
        "Review recurring expenses each quarter."
    ]
    suggestion = tips[hash(question) % len(tips)]

    return {
        "status": "ok",
        "analysis": f"Analysis for '{question}': {suggestion}"
    }
