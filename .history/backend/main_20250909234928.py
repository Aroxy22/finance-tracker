...
def analyze(payload: AskRequest):
        ...
        "contents": [
            {
                ...
                "parts": [
                    {
                        "text": f"Provide a financial analysis or advice based on this input: {question}"
                    }
                ],
            }
        ]
    ...
    try:
        ...
        analysis_text = "<div style='line-height:1.6; font-family:Segoe UI;'>"
        if "candidates" in result and len(result["candidates"]) > 0:
            parts = result["candidates"][0].get("content", {}).get("parts", [])
            if parts:
                raw_text = parts[0].get("text", "")
                ...
                analysis_text += "No analysis returned from Gemini."
        else:
            analysis_text += "No analysis returned from Gemini."
        analysis_text += "</div>"
