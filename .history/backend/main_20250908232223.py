...
def edit_transaction(payload: dict):
    ...
    if not tx:
        raise HTTPException(status_code=404, detail="transaction not found")
    tx.amount = payload.get("amount", tx.amount)
    tx.description = payload.get("description", tx.description)
    tx.category = payload.get("category", tx.category)
    tx.is_expense = payload.get("is_expense", tx.is_expense)
    tx.recurring = payload.get("recurring", tx.recurring)
    sess.commit()
    sess.refresh(tx)
    return {"status":"ok","transaction": {"id":tx.id,"amount":tx.amount,"description":tx.description,
                                          "category":tx.category,"is_expense":tx.is_expense,"recurring":tx.recurring}}
...
@app.post("/api/ask")
def ask_finance(payload: AskPayload):
    ...
    if hf_model is None or hf_tokenizer is None:
        try:
            ...
            import torch
            hf_tokenizer = AutoTokenizer.from_pretrained("mrm8488/bert-small-finetuned-finance")
            hf_model = AutoModelForCausalLM.from_pretrained("mrm8488/bert-small-finetuned-finance")
        except Exception as e:
            raise HTTPException(status_code=500, detail=f"Error loading HF model: {e}")

    # Include last 50 transactions as context
    sess = SessionLocal()
    txs = sess.query(Transaction).order_by(Transaction.ts.desc()).limit(50).all()
    context = "\n".join([f"{t.ts.date()} - {t.description}: {'-' if t.is_expense else '+'}${t.amount}" for t in txs])

    prompt = f"User question: {payload.text}\nTransactions:\n{context}\nAnswer concisely:"

    try:
        inputs = hf_tokenizer(prompt, return_tensors="pt", truncation=True, padding=True, max_length=512)
        with torch.no_grad():
            outputs = hf_model.generate(**inputs, max_length=200, num_return_sequences=1)
